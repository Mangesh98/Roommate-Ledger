...
export const getPendingRequestsAction = async (
	...
	try {
		const url = `${import.meta.env.VITE_HOST_URL}/entry/get-pending-requests`;
...
export const paymentRequestAction = async (entryId: string, token: string) => {
	try {
		const url = `${import.meta.env.VITE_HOST_URL}/entry/payment-request`;
